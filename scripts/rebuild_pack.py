...
Layout padrão deste repositório:
    - resource/  (contém assets/, pack.mcmeta, etc.)
    - scripts/   (contém este e outros utilitários)

Uso básico (from-images):
    # cria/atualiza o pack diretamente em ./resource
    python scripts/rebuild_pack.py --src "/caminho/para/imagens" --out ./resource --citresewn --pack-format 55
...
O que faz (modo padrão from-images):
    ...
    - Para cada skin, cria a estrutura CIT correta para Elytra, compatível com OptiFine e Fabric (CIT Resewn):
        ...
        (Opcional) espelha a mesma estrutura para:
            out/assets/minecraft/citresewn/cit/elytra/...
...
Modo reorganize-pack:
    - Se você já tem um pack (com pastas icon/model/properties), pode rodar:
            python scripts/rebuild_pack.py --mode reorganize-pack --out ./resource --citresewn --pack-format 55
...
# Caminhos padrão seguindo a nova estrutura (resource/ no topo do repo)
REPO_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_PACK_DIR = (REPO_ROOT / "resource")
...
if __name__ == '__main__':
    ...
    ap.add_argument("--src", required=False, help="Diretório de origem (imagens soltas OU pack existente). Para reorganize-pack, omitir usa ./resource")
    ap.add_argument("--out", required=False, default=str(DEFAULT_PACK_DIR), help="Diretório de saída do resource pack (padrão: ./resource)")
    ap.add_argument("--mode", choices=["from-images", "reorganize-pack"], default="from-images", help="Modo de operação")
    ap.add_argument("--citresewn", action="store_true", help="Espelhar para assets/minecraft/citresewn/cit (recomendado para Fabric)")
    ...
    if args.mode == "reorganize-pack":
        src = Path(args.src).expanduser().resolve() if args.src else out
    else:
        if not args.src:
            ap.error("--src é obrigatório no modo from-images")
